...
def test_delete_person_clears_embeddings_for_every_model(
    ...
    pid = created["id"]
    fake_vector_repo.embeddings.extend(
        [
            {
                "embed_id": f"e-{pid}-s",
                "crop_id": f"crop-{pid}-s",
                "person_id": pid,
    ...
    r = client.delete(f"/api/v1/persons/{pid}", headers={"X-API-Key": key})
    assert r.status_code == 200
